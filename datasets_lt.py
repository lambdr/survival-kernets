...
def load_dataset(dataset, random_seed_offset=0, fix_test_shuffle_train=False):
    ...
    if fixed_train_test_split: # This is only for the Rotterdam-GBSG data
        ...
        if fix_test_shuffle_train:
            ...
            X_train, X_test, y_train, y_test, l_train, l_test = \
                train_test_split(X, y, l, test_size=test_size,  # <-- add l
                                 random_state=rng)
            if random_seed_offset > 0:
                ...
                X_train = X_train[shuffled_indices]
                y_train = y_train[shuffled_indices]
                l_train = np.array(l_train, dtype='float32')[shuffled_indices]  # <-- same shuffle
        else:
            rng = np.random.RandomState(dataset_random_seed
                                        + random_seed_offset)
            X_train, X_test, y_train, y_test, l_train, l_test = \
    ...
    l_train = np.array(l_train, dtype='float32')  
    l_test = np.array(l_test, dtype='float32')

    return X_train, y_train, l_train, X_test, y_test, l_test, feature_names, \
