...
for experiment_idx in range(n_experiment_repeats):
    for dataset in datasets:
        X_train, y_train, l_train, X_test, y_test, l_test, feature_names, \
        ...
        l_proper_train = l_train[proper_train_idx].astype('float32')
        X_val = X_train[val_idx]
        y_val = y_train[val_idx].astype('float32')
        l_val = l_train[val_idx].astype('float32')
        ...
        l_proper_train = l_proper_train[keep_mask]
        ...
        if not os.path.isfile(output_train_metrics_filename) or \
            ...
            for hyperparam_idx, hyperparam in enumerate(finetune_hyperparams):
                ...
                model = NKS(net,
                            optimizer,
                            loss=NLLKernelHazardTruncationLoss(alpha,sigma,gamma),
                            alpha=alpha,
                            sigma=sigma,
                            gamma=gamma,
                            beta=beta,
                ...
                # map truncation times onto the same discrete grid
                l_proper_train_discrete = np.searchsorted(
                    label_transformer.cuts, l_proper_train, side='right') - 1
                l_proper_train_discrete = np.clip(
                    l_proper_train_discrete, 0,
                    len(label_transformer.cuts) - 1).astype(np.int64)
                train_pair = (X_proper_train_std,
                              (y_proper_train_discrete[0],
                               y_proper_train_discrete[1],
                               l_proper_train_discrete))
                train_loader = \
